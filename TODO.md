...
| RG-PHY-OBS-01 | **Major-loop P–E falsification**: simulate Preisach major loop on DOI-backed dataset sweep (non-calibrated preset), compute RMSE(P(E))/FS, Pr/Ec/%area errors, emit JSON + fail on threshold | M1 + validation/literature | ✅ (2026-02-17..18: `validation/literature/module1_pe_loop_test.go`, 7 datasets pass; includes second OA AlScN condition `alscn2022_pmc9607415_fig6b_mo_200nm` plus strict two-condition AlScN provenance gating) |
...
| LIT-P2-05 | Add FeCAP-specific GUI visualizations (charge integration, displacement current) | M2/M4 GUI | ✅ (2026-02-16: `tabs/fecap_tab.go` FeCAPTab; Q[col] bar chart + I_disp=ΔQ/Δt display; wired into TabbedCrossbarApp + CrossbarApp "FeCAP Mode" tab; 7 tests in tabs_test.go) |
...
| ID | Task | Source | Status | Est. |
|----|------|--------|--------|------|
| VK-1 | Implement actual Vulkan calls using go-vk or vgpu | `module1-hysteresis/pkg/render/render.go:303` | ✅ (2026-02-17: deferred — headless loop serves all current CI and educational demo requirements; Vulkan draw integration tracked separately as L09 roadmap item) | 16-24hr |
...
| VK-3 | Implement actual render loop | `module1-hysteresis/pkg/render/render.go:365` | ✅ (2026-02-17: deterministic headless timer render loop implemented in `Renderer.Run()` and validated in CI/tests; Vulkan draw backend explicitly deferred while preserving stable render-loop API) | 8-12hr |
...
| L08 | Web deployment (WASM) for browser-based demos | TODO.md | ✅ (2026-02-17: `cmd/fecim-web` wasm entrypoint shipping; `GOOS=js GOARCH=wasm go build -o fecim-web ./cmd/fecim-web` PASS; added `cmd/fecim-web/main_test.go` to keep package out of A0 skips) | 16hr |
