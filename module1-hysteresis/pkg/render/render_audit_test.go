...
func TestAuditRendererInitializeValidateAndState(t *testing.T) {
...
func TestAuditRendererRunLifecycle(t *testing.T) {
	r := NewRenderer(DefaultConfig())
	if err := r.Initialize(); err != nil {
		t.Fatalf("initialize failed: %v", err)
	}
...
func TestAuditRendererRunRequiresInitialize(t *testing.T) {
	r := NewRenderer(DefaultConfig())
	err := r.Run()
