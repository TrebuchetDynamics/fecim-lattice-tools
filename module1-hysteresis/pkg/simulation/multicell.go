...
func NewMultiCellArray(rows, cols int, material *ferroelectric.HZOMaterial) (*MultiCellArray, error) {
	if rows <= 0 || cols <= 0 { return nil, fmt.Errorf("invalid array dimensions: %dx%d", rows, cols) }
	if material == nil { return nil, fmt.Errorf("material cannot be nil") }
	...
	for r := 0; r < rows; r++ {
		...
		for c := 0; c < cols; c++ { m.models[r][c] = ferroelectric.NewPreisachModel(material) }
	}
	return m, nil
}
...
func (m *MultiCellArray) StepCell(row, col int, voltage float64) (CellState, error) {
	...
	if !m.inBounds(row, col) { return CellState{}, fmt.Errorf("cell index out of bounds: (%d,%d)", row, col) }
	return m.stepCellLocked(row, col, voltage), nil
}

func (m *MultiCellArray) StepWithVoltageMap(voltageMap [][]float64) error {
	m.mu.Lock(); defer m.mu.Unlock()
	if len(voltageMap) != m.rows { return fmt.Errorf("voltage map rows mismatch: got %d, want %d", len(voltageMap), m.rows) }
	for r := range voltageMap { if len(voltageMap[r]) != m.cols { return fmt.Errorf("voltage map cols mismatch at row %d: got %d, want %d", r, len(voltageMap[r]), m.cols) } }
	for r := 0; r < m.rows; r++ { for c := 0; c < m.cols; c++ { m.stepCellLocked(r, c, voltageMap[r][c]) } }
	return nil
}
...
func (m *MultiCellArray) StepWithSelector(cells []CellCoord, voltage float64) error {
	...
	for _, cell := range cells {
		if !m.inBounds(cell.Row, cell.Col) { return fmt.Errorf("cell index out of bounds: (%d,%d)", cell.Row, cell.Col) }
...
func (m *MultiCellArray) Snapshot() [][]CellState {
	...
	for r := 0; r < m.rows; r++ { out[r] = make([]CellState, m.cols); copy(out[r], m.states[r]) }
	return out
}

func (m *MultiCellArray) Reset() {
	m.mu.Lock(); defer m.mu.Unlock()
	for r := 0; r < m.rows; r++ { for c := 0; c < m.cols; c++ { m.models[r][c].Reset(); m.states[r][c] = CellState{} } }
}
...
func (m *MultiCellArray) inBounds(row, col int) bool { return row >= 0 && row < m.rows && col >= 0 && col < m.cols }
