...
2. Each card's 'Launch' button calls tabs.SelectIndex(N) or tabs.Select(tabItem)
...
DEMO 1 (Hysteresis) - VERIFY & POLISH:
...
- Fix N² counter to update with array size
- Make 'What You're Seeing' panel change per tab
- Test all buttons: Run MVM, Analyze IR Drop, Analyze Sneak Paths, Reset
- MUST expose BuildContent() for unified app

DEMO 3 (MNIST) - FIX TRAINING & ACCURACY:
...
- FIX: If accuracy is wrong, retrain network:
  ...
  - Target: 87% accuracy (match Dr. Tour's claim)
...
- Build temperature color scale (25°C blue to 85°C red)
...
- Build three-tab view:
  ...
  Tab C: Conductance Drift
    ...
    - Simulate: Level 15 → 14.9 (1hr) → 14.7 (1day) → 14.5 (1week)
...
- Build energy per MAC bar chart:
  - CPU + DRAM: ~1000 fJ/MAC (red bar)
  - GPU + HBM: ~100 fJ/MAC (orange bar)
  - FeCIM: ~1-10 fJ/MAC (green bar)
- Build architecture diagram:
  - Left: Von Neumann (CPU ↔ Memory, data movement arrows)
...
- Build data center calculator:
  ...
  - Output table:
    | Architecture | Power (W) | Cost ($/month) |
    | CPU Cluster  | X         | $X             |
    | GPU Cluster  | X         | $X             |
    | FeCIM        | X         | $X             |
  - Savings display: 'FeCIM saves X% vs GPU'
- Build verified vs claimed table:
  - ✅ VERIFIED: 30 levels, 87% MNIST, CMOS compatible, non-volatile
  - ⚠️ CLAIMED: 10M× vs NAND, 1000× vs DRAM, 80-90% savings
  - Source footnotes for each claim
- Add TRL status: 'Technology Readiness Level 4 (Lab Validation)'
- Add Dr. Tour quote: 'This could lower data center energy by 80 to 90%'
...
- Click card → tabs.Select(correctTab)
...
- Demo 3: Draw digit → correct prediction, ~87% accuracy
...
- Update quick start:
  ...
  ./fecim-visualizer
  ```
- Update demo table (all 8 marked ✅)
...
7. Demo 3: Drawing works, accuracy ~87% ✓
