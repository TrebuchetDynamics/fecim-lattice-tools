...
func captureOne(outDir, fileBase string, size fyne.Size, settle time.Duration, start bool, hysEngine string, create func() (sharedwidgets.EmbeddedApp, error)) error {
	...
	// Allow Fyne internal goroutines (catchTerm, etc.) to fully exit before
	// the next module creates a new app via app.NewWithID. Without this sleep,
	// a lingering catchTerm goroutine from the old app may call fyne.Do, which
	// dispatches to the new app's driver, causing "close of closed channel" panic.
