...
const (
	...
	crawlerScreenshotBaseDir    = "cmd/fecim-lattice-tools/testdata/screenshots/audit"
...
func runUICrawler(t *testing.T, config crawlerConfig, moduleFactory func() (moduleLifecycle, error)) {
	...
	if isHeadlessEnvironment() {
		// Use real app for headless (xvfb) mode to get proper rendering
		fy = app.New()
	} else {
		// Use real app for display mode as well for consistency
		fy = app.New()
	}
