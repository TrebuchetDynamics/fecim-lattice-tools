...
func runFyneApp(module viewmodel.ModuleID) error {
	requestedModule := fyneModuleName(module)
	if log == nil {
	...
	utils.SafeGo("startup-content-loader", func() {
		...
		fyne.Do(func() {
			...
			fmt.Printf("[STARTUP] Starting at module: %s (index %d)\n", requestedModule, startIdx)
			selectView(startIdx)
			log.Debug("Started at module %s (index %d)", requestedModule, startIdx)
...
func fyneModuleName(module viewmodel.ModuleID) string {
	switch module {
