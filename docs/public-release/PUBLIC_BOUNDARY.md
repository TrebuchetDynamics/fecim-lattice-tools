# Public Boundary

## Allowed
- Source code, tests, validation harnesses, safe example data
- Build/run/contributor documentation
- Research summaries written in project language
- DOI lists, arXiv links, metadata, and paper download scripts
- Third-party PDFs only when THIRD_PARTY_PDF_AUDIT.csv records keep or keep-with-conditions

## Disallowed
- docs/archive/**
- docs/4-research/internal-analysis/**
- docs/4-research/transcripts/COSM_2025_AI_Hardware_Breakthrough/**
- docs/4-research/transcripts/ironlattice-youtube-script.md
- docs/4-research/tour-group-ironlattice-research.md
- docs/4-research/superlattice-material-analysis.md
- Personal internal draft, research planning, restricted access/restricted material
- Third-party PDFs without explicit redistribution evidence
