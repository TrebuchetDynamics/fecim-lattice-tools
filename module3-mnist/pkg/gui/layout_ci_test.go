...
func TestModule3LayoutCI_NoOverlapAtCommonResolutions(t *testing.T) {
	app := fyneTest.NewApp()
	defer app.Quit()

	ma := NewMNISTApp()
	if ma == nil {
		t.Fatal("expected MNIST app instance")
	}

	w := app.NewWindow("m3-layout-ci")
	defer w.Close()

	root := ma.createMainLayout()
	...
	for _, size := range sizes {
		t.Run(fmt.Sprintf("%dx%d", int(size.Width), int(size.Height)), func(t *testing.T) {
			...
			nodes := flattenModule3Nodes(root)
			for _, n := range nodes {
				obj := n.obj
...
type module3NodeWithScroll struct {
	obj      fyne.CanvasObject
	inScroll bool
}

func module3PtrID(o fyne.CanvasObject) uintptr {
	v := reflect.ValueOf(o)
...
func flattenModule3Nodes(root fyne.CanvasObject) []module3NodeWithScroll {
	seen := map[uintptr]bool{}
	out := make([]module3NodeWithScroll, 0, 128)
	var walk func(fyne.CanvasObject, bool)
	walk = func(o fyne.CanvasObject, inScroll bool) {
		if o == nil {
			return
		}
		if p := module3PtrID(o); p != 0 {
			if seen[p] {
				return
			}
			seen[p] = true
		}
		out = append(out, module3NodeWithScroll{obj: o, inScroll: inScroll})
