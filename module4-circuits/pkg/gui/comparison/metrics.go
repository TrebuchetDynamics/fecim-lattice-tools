...
func ComputeMetrics(arraySize int) (MetricRow, MetricRow, MetricRow) {
	if arraySize <= 0 {
		arraySize = 8
	}
	...
	cpu := MetricRow{Label: "CPU", LatencyNS: 500 * scale, EnergyPJ: 64000 * scale}
	gpu := MetricRow{Label: "GPU", LatencyNS: 50 * scale, EnergyPJ: 6400 * scale}
	fefet := MetricRow{Label: "FeFET", LatencyNS: 76 * scale, EnergyPJ: 2.9 * scale}

	rows := []*MetricRow{&cpu, &gpu, &fefet}
	for _, r := range rows {
