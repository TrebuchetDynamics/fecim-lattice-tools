...
func (mc *MaterialCard) CreateRenderer() fyne.WidgetRenderer {
	...
	prText := materialdisplay.FormatPolarization(mc.material.PrCM2)
	ecText := materialdisplay.FormatField(mc.material.EcVM)
	mc.prLabel = widget.NewLabel(fmt.Sprintf("Pr: %s", prText))
	mc.ecLabel = widget.NewLabel(fmt.Sprintf("Ec: %s", ecText))

	// Reference (truncated)
	refText := materialdisplay.TruncateString(mc.material.Reference, 50)
...
func (r *materialCardRenderer) Refresh() {
	...
	r.card.prLabel.SetText(fmt.Sprintf("Pr: %s", materialdisplay.FormatPolarization(r.card.material.PrCM2)))
	r.card.ecLabel.SetText(fmt.Sprintf("Ec: %s", materialdisplay.FormatField(r.card.material.EcVM)))
	r.card.refLabel.SetText(materialdisplay.TruncateString(r.card.material.Reference, 50))
}
