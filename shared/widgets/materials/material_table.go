...
type MaterialTable struct {
	...
	properties []materialdisplay.FormattedProperty
...
func NewMaterialTable(material *physics.Material) *MaterialTable {
	mt := &MaterialTable{
		material:   material,
		properties: materialdisplay.GetMaterialProperties(material),
	}
...
func (mt *MaterialTable) SetMaterial(material *physics.Material) {
	...
	mt.properties = materialdisplay.GetMaterialProperties(material)
	mt.Refresh()
}
...
func (mt *MaterialTable) CreateRenderer() fyne.WidgetRenderer {
	...
	for _, category := range materialdisplay.CategoryOrder {
		catProps := materialdisplay.GetPropertiesByCategory(mt.properties, category)
		if len(catProps) == 0 {
			continue
		}
...
func (mt *MaterialTable) buildCategoryTable(props []materialdisplay.FormattedProperty) fyne.CanvasObject {
	if len(props) == 0 {
...
func (mdp *MaterialDetailPanel) SetMaterial(material *physics.Material) {
	...
	mdp.properties = materialdisplay.GetMaterialProperties(material)
	mdp.Refresh()
}
