...
func TestGenerateMVMSneakTrace(t *testing.T) {
	...
	if trace == nil || len(trace.Rows) != 4 || trace.TotalSneak <= 0 { t.Fatalf("invalid trace: %+v", trace) }
	if !strings.Contains(trace.FormatText(2, 1), "MVM Sneak Path Trace") { t.Fatal("missing trace header") }
}
...
func TestMVMWithNonIdealities_PopulatesSneakTrace(t *testing.T) {
	...
	opts := DefaultMVMOptions(); opts.EnableIRDrop = false; opts.EnableVariation = false; opts.EnableSneakPaths = true; opts.Architecture = "0T1R"
	res, err := arr.MVMWithNonIdealities([]float64{1, 1, 1, 1}, opts)
	if err != nil { t.Fatal(err) }
	if res.SneakTrace == nil { t.Fatal("expected SneakTrace") }
}
