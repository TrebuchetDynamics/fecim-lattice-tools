...
func MakeBuilderValidationTab(cfg *config.ArrayConfig, window fyne.Window) fyne.CanvasObject {
	...
	// Layout image display - shows KLayout-generated PNG
	// === THREE IMAGE DISPLAYS WITH SOURCE LABELS ===
	...
	klayoutImage.SetMinSize(fyne.NewSize(400, 300))
	...
	openroadCard := container.NewBorder(
		container.NewVBox(openroadLabel, openroadStatus), nil, nil, nil,
		openroadImage,
	)

	// Grid layout for all three images
	layoutStack := container.NewGridWithColumns(2,
		klayoutCard,
		yosysCard,
		openroadCard,
		widget.NewCard("Info", "", widget.NewLabel("All images generated by real EDA tools:\n\n"+
			"• KLayout: Reads DEF+LEF, exports PNG\n"+
			"• Yosys: 'show' command creates schematic SVG\n"+
			"• OpenROAD: 'save_image' exports placement PNG\n\n"+
			"Click 'Generate All' for KLayout image.\n"+
			"Use buttons above for Yosys/OpenROAD.")),
	)
	...
	// Helper to update Yosys schematic image
	updateYosysImage := func() {
		svgPath := fmt.Sprintf("output/exports/fecim_crossbar_%dx%d_schematic.svg", cfg.Rows, cfg.Cols)
		absPath, _ := filepath.Abs(svgPath)
		if fileExists(absPath) {
			yosysImage.File = absPath
			yosysImage.Resource = nil
			fyne.Do(func() {
				yosysStatus.SetText("Generated: " + filepath.Base(svgPath))
				yosysImage.Refresh()
			})
		}
	}
		...
		go func() {
			...
			fyne.Do(func() {
				if result != nil && result.Success {
					...
					dialog.ShowError(fmt.Errorf("Schematic generation failed: %s", errMsg), window)
				}
			})
		}()
	})
		...
		go func() {
			...
			fyne.Do(func() {
				if result != nil && result.Success {
					...
					dialog.ShowError(fmt.Errorf("Layout generation failed: %s", errMsg), window)
				}
			})
		}()
	})
...
`, designName, time.Now().Format("2006-01-02"), designName, designName, designName)
	...
	// Layout tab with real EDA tool images only (no custom SVG)
	layoutHelp := widget.NewLabel("Real EDA images only: KLayout (auto) | Yosys | OpenROAD")
